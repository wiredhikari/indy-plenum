...
from plenum.server.plugin.did_plugin.request_handlers.update_network_did import UpdateNetworkDIDRequest
...
def integrate_plugin_in_node(node):
    ...
    node.write_manager.register_req_handler(UpdateNetworkDIDRequest(node.db_manager, did_dict))
    # node.write_manager.register_req_handler(CreateSDDIDHandler(node.db_manager, did_dict))
